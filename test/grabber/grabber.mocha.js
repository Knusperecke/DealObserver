...
describe('Grabber', () => {
  ...
  function createGrabber({
    ...
    pushMock = sinon
      .stub()
      .returns({ offerIds: [], newOffers: [], priceUpdates: [] }),
    ...
    config.databaseMock = sinon.stub().returns({
      close: config.closeDatabase,
      push: config.push,
      updateCurrent: config.updateCurrent,
    });
    ...
    config.grabberConfig = {
      database: { host: '', user: '', password: '', table: '' },
    };
  ...
  describe('Data flow', () => {
    ...
    it('Passes offerIds from database.push to database.updateCurrent', async () => {
      ...
      const pushMock = sinon
        .stub()
        .returns(
          Promise.resolve({ offerIds: [123], newOffers: [], priceUpdates: [] }),
        );
    ...
    it('Passes new offers to updatePreprocessor', async () => {
      ...
      const pushMock = sinon
        .stub()
        .returns(
          Promise.resolve({ offerIds: [], newOffers: [123], priceUpdates: [] }),
        );
      await createGrabber({ fetcherMock, parserMock, pushMock });
    ...
    it('Passes price updates to updatePreprocessor', async () => {
      ...
      const pushMock = sinon
        .stub()
        .returns(
          Promise.resolve({ offerIds: [], newOffers: [], priceUpdates: [123] }),
        );
      await createGrabber({ fetcherMock, parserMock, pushMock });
    ...
    it('Passes items that disappeared to updatePreprocessor', async () => {
      ...
      const pushMock = sinon
        .stub()
        .returns(
          Promise.resolve({ offerIds: [], newOffers: [], priceUpdates: [] }),
        );
      const updateCurrentMock = sinon
        .stub()
        .returns(Promise.resolve([expectedItem]));
      await createGrabber({
        fetcherMock,
        parserMock,
        pushMock,
    ...
    it('Passes multiple items that disappeared to updatePreprocessor', async () => {
      ...
      const updateCurrentMock = sinon
        .stub()
        .returns(Promise.resolve(expectedItems));
      await createGrabber({
        fetcherMock,
        parserMock,
        pushMock,
  ...
  describe('Error handling', () => {
    it('Catches errors during the data flow', async () => {
      ...
      const parserMock = sinon
        .stub()
        .returns(Promise.reject(new Error('Parsing failed')));
      await createGrabber({ fetcherMock, parserMock });
    });
