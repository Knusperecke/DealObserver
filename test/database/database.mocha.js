  ...
  describe('Push items', () => {
    ...
    it('"push" function provides a price update for permanent items', async () => {
      ...
      await db.push([newPermanentItemUpdatedPrice]).then((updates) => {
        ...
        assert.deepEqual(
          updates.priceUpdates[0].item,
          newPermanentItemUpdatedPrice,
        );
        assert.deepEqual(
          updates.priceUpdates[0].oldPrice,
          newPermanentItem.price,
        );
        assert.deepEqual(
          updates.priceUpdates[0].newPrice,
    ...
    it('"push" function updates all current items if one item matches multiple similar ones (tradeoff if we cannot discern them))', async () => {
      ...
      await db
        .push([newOutletItem, newOutletItemIdenticalOnDbLayer])
        .then(async (updates) => {
          assert.isObject(updates);
          assert.deepEqual(updates.priceUpdates.length, 0);
          assert.deepEqual(updates.newOffers.length, 2);
  ...
  describe('Update current items', () => {
    it('"updateCurrent" with empty array must empty the "current" table', async () => {
      ...
      await db.updateCurrent([]).then(async () => {
        await db
          .query('SELECT COUNT(historyId) AS count FROM current')
          .then((results) => {
    ...
    it('"updateCurrent" must handle offerIds returned by "push"', async () => {
      ...
      await db
        .push([newOutletItem, newPermanentItem])
